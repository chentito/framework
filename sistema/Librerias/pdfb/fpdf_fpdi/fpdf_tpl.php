...
class fpdf_tpl extends fpdf {
    	...
    	{                
    		...
    		if(isset($this->CoreFonts[$fontkey]))
    		{
    			if(!isset($fpdf_charwidths[$fontkey]))
    			{
    				//Load metric file
    				$file=$family;
    				if($family=='times' or $family=='helvetica' or $family=='arial' or $family=='courier') 
    					$file.=strtolower($style);
    				$file.='.php';
    				if(defined('FPDF_FONTPATH'))
    					$file=FPDF_FONTPATH.$file;
    				include(_DIRPATH_."/".$file);
    				if( !isset($fpdf_charwidths[$fontkey]) )
    					$this->Error('Could not include font metric file '.$file.' /fontkey/ '.$fontkey);
    			}
                $i = $this->findNextAvailFont();
                $this->fonts[$fontkey]=array('i'=>$i,'type'=>'core','name'=>$this->CoreFonts[$fontkey],'up'=>-100,'ut'=>50,'cw'=>$fpdf_charwidths[$fontkey]);
    		}
    		else
    			$this->Error('Undefined font: '.$family.' '.$style.' esto busca '.$fontkey);
    	}
    	//Select it
    ...
    function AddPage($orientation='') {
        ...
        $this->setSourceFile(_DIRPATH_."/sistema/Archivos/background/Vacio.pdf");
